...
setwd("~/Documents/Orchard_Common_Garden/commongarden")
...
OCG_GC_2012 <- read.csv("data_csv/OCG_2012_GC.csv", head=T,check.names = F,stringsAsFactors = T, skip = 1) #183 obs of 221 var

## 2012 GC Cleaning 
# Remove the second column since it is empty
OCG_GC_2012 <- OCG_GC_2012[, -2]
...
OCG_GC_2012 <- OCG_GC_2012 [, grepl("Peak.Area|Plant_ID", colnames(OCG_GC_2012))] #167 obs of 147 variables

#subset to remove columns that contain Peak Area Percent and just keep Peak Area.
OCG_GC_2012 <- OCG_GC_2012 [, !grepl("Peak.Area.Percent", colnames(OCG_GC_2012))] #167 obs of 74 variables
...
OCG_GC_2012 <- merge(md.OCG.2012, OCG_GC_2012, by="Garden Plant ID") #157 obs of 89

OCG_GC_2012 <- OCG_GC_2012[,-c(1:15)] #removing everything except area under the curve 157 obs of 74 variables
...
OCG_GC_2021 <- OCG_GC_2021[,-c(1:15)] #70 obs of 74 variables
...
sum(duplicated(OCG_LCMS_1uL$Plant_ID)) #1 duplicate: plant #273
...
OCG_LCMS_1uL <- OCG_LCMS_1uL[-8, ] #119 of 930
OCG_LCMS_1uL <- OCG_LCMS_1uL[-61, ] #118 of 930
...
names(OCG_LCMS_1uL)[names(OCG_LCMS_1uL) == 'Plant_ID_Number'] <- 'Garden Plant ID'

#2012#
OCG_LCMS_1uL_2012 <- subset(OCG_LCMS_1uL, OCG_LCMS_1uL$Year=="2012") #45 observations and 311 variables

md.OCG.2012$`Garden Plant ID` <- as.character(md.OCG.2012$`Garden Plant ID`) 

OCG_LCMS_1uL_2012 <- merge(md.OCG.2012, OCG_LCMS_1uL_2012, by="Garden Plant ID") #40 obs of 326 variables
...
OCG_LCMS_1uL_2021 <- merge(md.OCG.2021, OCG_LCMS_1uL_2021, by="Garden Plant ID") #70 of 326 var
...
## 2012 CLEAN GC
OCG_GC_2012 <- read.csv("data_csv/OCG_GC_2012_cleaned.csv", row.names = 1) #157 obs of 73 variables
OCG_GC_2012 <- OCG_GC_2012[order(row.names(OCG_GC_2012)),]
OCG_GC_2012 <- subset(OCG_GC_2012, row.names(OCG_GC_2012) %in% row.names(md.OCG)) #147 of 73 variables
OCG_GC_2012[is.na(OCG_GC_2012)] <- 0

## 2021 CLEAN GC
OCG_GC_2021 <- read.csv("data_csv/OCG_GC_2021.csv", row.names = 1)#70 obs of 73 variables
OCG_GC_2021 <- OCG_GC_2021[order(row.names(OCG_GC_2021)),] 
OCG_GC_2021 <- subset(OCG_GC_2021, row.names(OCG_GC_2021) %in% row.names(md.OCG)) #70 of 73 variables
OCG_GC_2021[is.na(OCG_GC_2021)] <- 0

##FULL CLEAN GC
OCG_GC <- rbind(OCG_GC_2012, OCG_GC_2021) #218 obs of 73 variables
OCG_GC <- OCG_GC[order(row.names(OCG_GC)),]
OCG_GC <- subset(OCG_GC, row.names(OCG_GC) %in% row.names(md.OCG)) #217 of 73 variables
...
# OCG_LCMS_1uL <- OCG_LCMS_1uL[order(row.names(OCG_LCMS_1uL)),] 
# OCG_LCMS_1uL <- subset(OCG_LCMS_1uL, row.names(OCG_LCMS_1uL) %in% row.names(md.OCG)) #109 of 310 variables
...
## 1uL LCMS data cleaning: USE 3UL INSTEAD 
# OCG_LCMS_1uL[OCG_LCMS_1uL == 0] <- NA
# colSums(is.na(OCG_LCMS_1uL))
# 
...
# 
# colSums(is.na(OCG_LCMS_1uL_subset))
...
##1UL LCMS SCALING
# data_LCMS1_normalized <- scale(OCG_LCMS_1uL_subset) #1:109, 1:278
# #Plant ID
# OCG_LCMS_1uL.t <- t(OCG_LCMS_1uL_subset) 
# colSums(is.na(OCG_LCMS_1uL.t))
# data_LCMS1_normalized.ID <- scale(OCG_LCMS_1uL.t) #1:278, 1:109 
...
#Compound
# corr_matrix_LCMS1 <- cor(data_LCMS1_normalized) #large matrix 77284 elements
# #Plant ID
# corr_matrix_LCMS1.ID <- cor(data_LCMS1_normalized.ID) #109
...
# fviz_cos2(data.pca_LCMS1.ID, choice = "var", axes = 1:2) 
# fviz_cos2(data.pca_LCMS1.ID, choice = "ind", axes = 1:2)
...
plotcolor <- c("olivedrab","cadetblue","magenta","blue","pink","green","darkgreen","darkred","goldenrod","yellow","cornflowerblue")

plot(data.pca_LCMS3_ID$x[, 1], data.pca_LCMS3_ID$x[, 2],
     xlab="PC 1", ylab="PC 2", 
     main="LCMS 3uL plant by Location", 
     col= plotcolor[md.OCG$Location],
     pch=c(16),
     xlim = range(data.pca_LCMS3_ID$x[, 1], na.rm = TRUE),
     ylim = range(data.pca_LCMS3_ID$x[, 2], na.rm = TRUE))
legend("topleft", 
...
str(md.OCG.2012)
#OCG_GC_2012[is.na(OCG_GC_2012)] <- 0
OCG_GC_2012.t <- t(OCG_GC_2012) #transpose for Plant ID
#OCG_GC_2012.t <- rarefy(OCG_GC_2012.t)
m_OCG_AUC_2012.t = as.matrix(OCG_GC_2012.t)

set.seed(37)
#OCG_AUC_2012_ID.nmds <- metaMDS(t(m_OCG_GC_2012.t), trymax=1000) #solution reached
...
OCG_GC_2012_ploidy <- adonis2(OCG_GC_2012 ~ md.OCG.GC.2012$Ploidy,by="margin") 
OCG_GC_2012_ploidy #ploidy is significant 0.002
...
OCG_GC_2012_subsp <- adonis2(OCG_GC_2012 ~ md.OCG.GC.2012$Subspecies,by="margin") OCG_GC_2021
...
OCG_GC_2012_subspploidy <- adonis2(OCG_GC_2012 ~ md.OCG.GC.2012$Subsp_ploidy,by="margin") 
...
OCG_GC_2021.t <- t(OCG_GC_2021)
m_OCG_AUC_2021.t = as.matrix(OCG_GC_2021.t)
...
OCG_GC_2021_ploidy <- adonis2(OCG_GC_2021 ~ md.OCG.GC.2021$Ploidy,by="margin")
...
#### PERMANOVA FOR SUBSPECIES
OCG_GC_2021_subsp <- adonis2(OCG_GC_2021 ~ md.OCG.GC.2021$Subspecies,by="margin") 
OCG_GC_2021_subsp #subspecies is signficant= 0.001

#PAIRWISE ADONIS SUBSPECIES
OCG_GC_2021_subsp.pw <- pairwise.adonis(OCG_GC_2021, md.OCG.GC.2021$Subspecies)
...
# PERMANOVAS FOR SUBSPECIES PLOIDY
OCG_GC_2021_subspploidy <- adonis2(OCG_GC_2021 ~ md.OCG.GC.2021$Subsp_ploidy,by="margin") 
OCG_GC_2021_subspploidy #subspecies ploidy is significant

#PAIRWISE ADONIS FOR SUBSPECIES PLOIDY
OCG_GC_2021_subsp_ploidy.pw <- pairwise.adonis(OCG_GC_2021, md.OCG.GC.2021$Subsp_ploidy)
...
plot(OCG_GC_ID.nmds, type = "t", display = "sites", cex = .4)

md.OCG.GC <- subset(md.OCG, row.names(md.OCG) %in% row.names(OCG_GC)) #217
OCG_GC <- subset(OCG_GC, row.names(OCG_GC) %in% row.names(md.OCG)) #217
...
OCG_GC_ploidy <- adonis2(OCG_GC ~ md.OCG.GC$Ploidy,by="margin")
...
#### PERMANOVA FOR SUBSPECIES###
OCG_GC_subsp <- adonis2(OCG_GC ~ md.OCG.GC$Subspecies,by="margin") 
OCG_GC_subsp #subspecies is signficant= 0.001

OCG_GC_subsp_yr <- adonis2(OCG_GC ~ md.OCG.GC$Subspecies*md.OCG.GC$Year,by="margin") 
OCG_GC_subsp_yr #sig 
...
# PERMANOVAS FOR SUBSPECIES PLOIDY
OCG_GC_subspploidy <- adonis2(OCG_GC ~ md.OCG.GC$Subsp_ploidy,by="margin") 
OCG_GC_subspploidy #subspecies ploidy is significant

OCG_GC_subspploidy_yr <- adonis2(OCG_GC ~ md.OCG.GC$Subsp_ploidy+md.OCG.GC$Year,by="margin")
...
OCG_GC_yr <- adonis2(OCG_GC ~ md.OCG.GC$Year,by="margin") 
...
# 
# md.OCG.LCMS.1 <- subset(md.OCG, row.names(md.OCG) %in% row.names(OCG_LCMS_1uL)) #109
#
...
# 
# #### PERMANOVA for ploidy ##
# OCG_LCMS1_ploidy <- adonis2(OCG_LCMS_1uL ~ md.OCG.LCMS.1$Ploidy,by="margin") 
# OCG_LCMS1_ploidy #ploidy is significant 0.001
# 
# OCG_LCMS1_loc <- adonis2(OCG_LCMS_1uL ~ md.OCG.LCMS.1$Location,by="margin") 
...
# OCG_LCMS1_subsp <- adonis2(OCG_LCMS_1uL ~ md.OCG.LCMS.1$Subspecies,by="margin") 
...
# 
# # PERMANOVAS 
# OCG_LCMS1_subspploidy <- adonis2(OCG_LCMS_1uL ~ md.OCG.LCMS.1$Subsp_ploidy ,by="margin") 
...
# 
# # PERMANOVAS for year ##
# OCG_LCMS1_year <- adonis2(OCG_LCMS_1uL ~ md.OCG.LCMS.1$Year,by="margin") 
...
#### PERMANOVA PLOIDY
OCG_LCMS3_ploidy <- adonis2(OCG_LCMS_3uL ~ md.OCG.LCMS.3$Ploidy ,by="margin") 
...
#### PERMANOVA FOR SUBSPECIES
OCG_LCMS3_subsp <- adonis2(OCG_LCMS_3uL ~ md.OCG.LCMS.3$Subspecies,by="margin") 
...
# SUBSPECIES PLOIDY 
plot(OCG_LCMS_3uL_ID.nmds$points, xlab="NMDS Axis 1", ylab="NMDS Axis 2", 
     main="LCMS 3uL chemistry by subspecies ploidy", 
...
# PERMANOVAS FOR SUBSPECIES PLOIDY
OCG_LCMS3_subspploidy <- adonis2(OCG_LCMS_3uL ~ md.OCG.LCMS.3$Subsp_ploidy + md.OCG.LCMS.3$Location ,by="margin") 
...
# PERMANOVAS
OCG_LCMS3_year <- adonis2(OCG_LCMS_3uL ~ md.OCG.LCMS.3$Year ,by="margin") 
...
#OCG_LCMS_2012.t <- t(OCG_LCMS_2012) 
